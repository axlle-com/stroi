...
OwlCarousel.prototype.add_events = function () {
    ...
    function drag_on(e) {
        e.preventDefault();
        e.stopPropagation();
        //e.pageX  ? self.point =  e.pageX : self.point =  e.touches[0].pageX;
        //e.type === 'mousedown' ? self.point = e.pageX : self.point = e.touches[0].pageX;
        self.point =  e.pageX;
        self.drag();
    }
    function drag_on_m(e) {
        e.preventDefault();
        e.stopPropagation();
        self.point =  e.touches[0].pageX;
        self.drag_m();
    }
    //var clickEvent =('ontouchstart'in window ?'touchend' : 'mousedown');
    //this.carousel_stage.addEventListener(clickEvent,drag_on,false);

    this.carousel_stage.addEventListener('mousedown',drag_on,false);
    this.carousel_stage.addEventListener('touchstart',drag_on_m,false);
};
...
OwlCarousel.prototype.drag_m = function () {
    var self = this;
    ...
    var box;
    box = self.carousel_stage.getBoundingClientRect();
    var left = box.left - self.carousel_stage_size;
    var right = box.right+self.carousel_stage_size-this.margin;
    var x;
    function move_box(e) {
        e.preventDefault();
        e.stopPropagation();
        var box1;
        box1 = self.carousel_stage.getBoundingClientRect();
        self.shift_left = box1.left - self.carousel_stage_size;
        var shift_right = box1.right+self.carousel_stage_size-self.margin;
        x = e.touches[0].pageX-self.point;
        //e.type == 'mousemove' ? x = e.pageX-self.point : x = e.touches[0].pageX-self.point;
        var coof = 2;
        console.log('m-'+left+'/'+self.shift_left);
        if(self.shift_left > 0 && x > 0){
            self.carousel_stage.style.transform = 'translate3d('+(self.shift_left+(self.shift_left/(self.shift_left*5)))+'px, 0px, 0px)';
        }else if(shift_right < self.my_width && x < 0){
            self.carousel_stage.style.transform = 'translate3d('+(self.shift_left-(self.shift_left/(self.shift_left*5)))+'px, 0px, 0px)';
        }else{
            self.carousel_stage.style.transform = 'translate3d('+(left + x/coof)+'px, 0px, 0px)';
        }
    ...
    function move_box_end(e) {
        e.preventDefault();
        e.stopPropagation();
        var cnt = Math.abs(self.shift_left/self.item);
        if(x < 0){
...
OwlCarousel.prototype.drag = function () {
    var self = this;
    this.carousel_stage.addEventListener('mousemove',move_box,false);
    //this.carousel_stage.addEventListener('touchmove',move_box,false);
    document.addEventListener('mouseup',move_box_end,false);
    ...
    var box = self.carousel_stage.getBoundingClientRect();
    var left = box.left - self.carousel_stage_size;
    var right = box.right+self.carousel_stage_size-this.margin;
    var shift_left;
    var x;
    function move_box(e) {
        ...
        box = self.carousel_stage.getBoundingClientRect();
        shift_left = box.left - self.carousel_stage_size;
        var shift_right = box.right+self.carousel_stage_size-self.margin;
        x = e.pageX-self.point;
        //e.type == 'mousemove' ? x = e.pageX-self.point : x = e.touches[0].pageX-self.point;
        var coof = 2;
        console.log('d/'+left+'/'+shift_left);
        if(shift_left > 0 && x > 0){
            self.carousel_stage.style.transform = 'translate3d('+(shift_left+(shift_left/(shift_left*5)))+'px, 0px, 0px)';
        }else if(shift_right < self.my_width && x < 0){
            self.carousel_stage.style.transform = 'translate3d('+(shift_left-(shift_left/(shift_left*5)))+'px, 0px, 0px)';
        }else{
            self.carousel_stage.style.transform = 'translate3d('+(left + x/coof)+'px, 0px, 0px)';
    ...
    function move_box_end(e) {
        e.preventDefault();
        e.stopPropagation();
        var cnt = Math.abs(shift_left/self.item);
        if(x < 0){
            if(cnt > (self.owl_items.length - self.cnt)){
                self.translation = (self.owl_items.length - self.cnt);
            }else {
